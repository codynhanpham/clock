...
function updateClock() {
    getTime().then(now => {
        let time = now.time;
        let ms = now.ms;
        let date_long = now.date_long;

        let clockTime = document.getElementById('hms');
        let clockMs = document.getElementById('ms');
        let clockDate = document.getElementById('date_text');

        // only update html if time or date has changed
        if (clockTime.innerHTML !== time) {
            clockTime.innerHTML = time;
        }
        if (clockMs.innerHTML !== ms) {
            clockMs.innerHTML = ms;
        }
        if (clockDate.innerHTML !== date_long) {
            clockDate.innerHTML = date_long;
        }
...
function toggleDate() {
    ...
    if (dateLine.classList.contains('hidden')) {
        ...
        setTimeout(function () {
            ...
            dateText.classList.remove('visuallyhidden');
        }, 50);
        console.log("Date is shown");
    } else {
        dateLine.classList.add('visuallyhidden');    
        dateText.classList.add('visuallyhidden');    
...
// also pause clock update when page is hidden
